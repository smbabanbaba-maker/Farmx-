...
interface Props {
  ...
  onPaid?: (via: "wallet" | "paystack", reference: string) => void | Promise<void>;
}
...
export function PayModal({
  ...
  const payWithWallet = async () => {
    ...
    try {
      ...
      await onPaid?.("wallet", `wallet_${Date.now()}`);
      onClose();
    } catch (error) {
      setMsg(error instanceof Error ? error.message : "An kasa kammala biyan kuɗi.");
    } finally {
      setBusy(false);
    }
  };
  ...
  const payWithPaystack = async () => {
    ...
    try {
      ...
      await new Promise((resolve) => setTimeout(resolve, 700));
      await onPaid?.("paystack", res.reference);
      onClose();
    } catch (e) {
